...
export class Foundation {
  ...
  */
   ...
   ngOnInit() {
     ...
     const maxWtg = Math.max(this.wtgQty, ...this.summaryRows.map((r:any) => r.totalWtgs));
     for (let i = 1; i <= maxWtg; i++) {
       this.wtgRows.push({
         wtgId: `WTG-${String(i).padStart(2, '0')}`,
         wtgCode: `WTG-${String(i).padStart(2, '0')}`,
   ...
   openViewAllDataFromWtg() {
     this.viewVisible = true;
     let data = {
       "projectId": this.selectedSummaryRow?.projectId
     }
   ...
   getViewWtgRows(): WtgRow[] {
     // const count = this.viewAllData?.totalWtgs ?? this.wtgRows.length;
     const count = this.viewAllData.length;
     return this.viewAllData.slice(0, count);
   }
